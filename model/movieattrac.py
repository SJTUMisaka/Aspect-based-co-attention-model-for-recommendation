...
from .attentionlayer import BatchAttention, BiasedAttention, Attention, AverageAttention, CoAttention, AlternatingCoAttention
...
class MovieAttractionModel(object):
    ...
    def rebuild(self):
        self.score_model, self.contrast_model = self._build()

    def _build(self):
        u_input = Input(shape=(1,), dtype='int32', name='user_index')
        #word_input = Input(shape=(self.max_sent_len,), dtype='int32', name='word_index')
        sent_input = Input(shape=(self.max_sent_len,), dtype='int32', name='sentence_index')
        #sent_encoder = None
        ...
        w_embed = self.word_embedding(sent_input)
        ...
        sent_embed_att = BatchAttention(name='word_attention', alpha=self.alpha_att_word, keepdims=True)([w_embed, w_user_embed])
        sent_embed_avg = Attention(name='word_avg_attention', alpha=self.alpha_word, keepdims=True)(w_embed)
        ...
        co_result = CoAttention(name = 'sent_co_attention', keepdims=True)([actor_embed, w_embed])
        sent_embed_co = co_result[1]
        sent_embed = concatenate([sent_embed_co, sent_embed_avg])
        #sent_embed = concatenate([sent_embed_att, sent_embed_co, sent_embed_avg])
        #sent_encoder = Model([u_input, word_input], sent_embed, name='model_sent_encoder')
        cast_embedding_co = co_result[0]
        cast_embedding = concatenate([cast_embedding_co, cast_embedding_avg])
        #cast_embedding = concatenate([cast_embedding_att, cast_embedding_co, cast_embedding_avg])
        cast_embedding = Dense(self.cast_embed_len, activation=self.hid_activation)(cast_embedding)


        joint_embedding = concatenate([sent_embed, cast_embedding], name='concat_embedding')
        ...
        pos_doc_input = Input(shape=(self.max_sent_len,), dtype='int32', name="pos_doc_input")
        neg_doc_input = Input(shape=(self.max_sent_len,), dtype='int32', name="neg_doc_input")
        ...
        return score_model, contrastive_model

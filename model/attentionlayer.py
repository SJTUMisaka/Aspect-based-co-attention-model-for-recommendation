...
class CoAttention(ISRAttentionLayer):
    def __init__(self, keepdims=False, **kwargs):
        self.supports_masking = True
        super(CoAttention, self).__init__(keepdims, **kwargs)
    ...
    def compute_output_shape(self, input_shape):
        assert isinstance(input_shape, list)
        return [(input_shape[0][0], 1) + input_shape[0][2:], (input_shape[1][0], 1) + input_shape[1][2:]]
...
class AlternatingCoAttention(ISRAttentionLayer):
    def __init__(self, keepdims=False, mid_units=200, **kwargs):
        self.supports_masking = True
        self.mid_units = mid_units
        super(AlternatingCoAttention, self).__init__(keepdims, **kwargs)

    def build(self, input_shape):
        assert isinstance(input_shape, list)
        self.wx_1 = self.add_weight(shape=(input_shape[0][-1], self.mid_units),
                                      name='wx_1',
                                      initializer='uniform',
                                      trainable=True)
        self.whx_1 = self.add_weight(shape=(self.mid_units, 1),
                                      name='whx_1',
                                      initializer='uniform',
                                      trainable=True)

        self.wx_2 = self.add_weight(shape=(input_shape[1][-1], self.mid_units),
                                      name='wx_2',
                                      initializer='uniform',
                                      trainable=True)
        self.whx_2 = self.add_weight(shape=(self.mid_units, 1),
                                      name='whx_2',
                                      initializer='uniform',
                                      trainable=True)
        self.wg_2 = self.add_weight(shape=(input_shape[1][-1], self.mid_units),
                                      name='wg_2',
                                      initializer='uniform',
                                      trainable=True)
        self.wx_3 = self.add_weight(shape=(input_shape[0][-1], self.mid_units),
                                      name='wx_3',
                                      initializer='uniform',
                                      trainable=True)
        self.whx_3 = self.add_weight(shape=(self.mid_units, 1),
                                      name='whx_3',
                                      initializer='uniform',
                                      trainable=True)
        self.wg_3 = self.add_weight(shape=(input_shape[0][-1], self.mid_units),
                                      name='wg_3',
                                      initializer='uniform',
                                      trainable=True)
        super(AlternatingCoAttention, self).build(input_shape)
    ...
    def call(self, inputs):
        assert isinstance(inputs, list)
        H1 = K.dot(inputs[0], self.wx_1)
        H1 = activations.tanh(H1)
        ax1 = K.dot(H1, self.whx_1)
        eax1 = K.exp(ax1)
        eax1 /= K.sum(eax1, axis = 1, keepdims = True)
        x1 = tf.matmul(tf.transpose(inputs[0], perm = [0, 2, 1]), eax1)
        
        H2 = K.dot(inputs[1], self.wx_2)
        H2 = H2 + K.dot(tf.transpose(x1, perm = [0, 2, 1]), self.wg_2)
        H2 = activations.tanh(H2)
        ax2 = K.dot(H2, self.whx_2)
        eax2 = K.exp(ax2)
        eax2 /= K.sum(eax2, axis = 1, keepdims = True)
        x2 = tf.matmul(tf.transpose(inputs[1], perm = [0, 2, 1]), eax2)
        
        H3 = K.dot(inputs[0], self.wx_3,)
        H3 = H3 + K.dot(tf.transpose(x2, perm = [0, 2, 1]), self.wg_3)
        H3 = activations.tanh(H3)
        ax3 = K.dot(H3, self.whx_3)
        eax3 = K.exp(ax3)
        eax3 /= K.sum(eax3, axis = 1, keepdims = True)
        x3 = tf.matmul(tf.transpose(inputs[0], perm = [0, 2, 1]), eax3)
       
        output0 = tf.transpose(x3, perm = [0, 2, 1])
        output1 = tf.transpose(x2, perm = [0, 2, 1])

...
for k,v in doc_cast_list.items():
    padding_doc = np.zeros(max_sent_len, dtype='int32')
    ...
    if temp: padding_doc[-len(temp):] = temp
...
from model.attentionlayer import BatchAttention, BiasedAttention, Attention, AverageAttention, CoAttention, AlternatingCoAttention
...
def test():
    ...
    AUC_list.append(np.mean(auclist))
    print("MAP:")
    print(ap / nTest)
    print("nDCG:")
    print(ndcg / nTest)
...
def train_with_test(train_set, testepoch=2):
    #model.contrast_model.load_weights("MovieLens_Model.hd5", by_name=True)      #load
...
train_with_test(train_set, 1)

print (AUC_list)
plt.plot(x_list, AUC_list)
plt.show()